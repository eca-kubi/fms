...
/*
    ...
    $('.bs-select')
        ...
        .on('loaded.bs.select show.bs.select', function () {
            ...
            $('.bs-select').each(function (index) { $(this).attr('tabindex', ++index) })
        });

    $('select.has-other-type').on('changed.bs.select', function () {
        let $this = $(this);
        var $otherType;
        if ($this.has('option[value=Other]:selected').length) {
            $otherType = $this.parents('.row').find(`#other_${$this.attr('id')}`);
            $otherType.parents('.col-sm-6').removeClass('d-none');
            $otherType.attr('required', 'true');
            $this.parents('form').validator('update');
        } else {
            $otherType = $this.parents('.row').find(`#other_${$this.attr('id')}`);
            $otherType.parents('.col-sm-6').addClass('d-none');
...
window.addEventListener("load", function () {
    ...
    $(document).on('click', '#stopProcess .btn-primary', null, stopChangeProcess);
